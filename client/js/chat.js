...
function renderFileMessage(fileData, isSender) {
	...
	// Check if we have media preview available
	const transfer = window.fileTransfers ? window.fileTransfers.get(fileId) : null;
	const hasPreview = transfer && transfer.previewUrl && transfer.mediaType && transfer.status === 'completed';
	...
	// Generate media preview HTML if available
	let mediaPreviewHtml = '';
	if (hasPreview) {
		if (transfer.mediaType === 'image') {
			// Image preview - clickable to open in modal
			mediaPreviewHtml = `
				<div class="file-media-preview">
					<img src="${transfer.previewUrl}" 
					     alt="${safeFileName}" 
			...
			mediaPreviewHtml = `
				<div class="file-media-preview">
					<video controls class="file-preview-video" preload="metadata">
						<source src="${transfer.previewUrl}" type="${transfer.mimeType || 'video/mp4'}">
