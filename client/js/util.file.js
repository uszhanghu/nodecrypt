...
// 将多个文件压缩为单个分卷归档
async function compressFilesToArchive(files, volumeSize = DEFAULT_VOLUME_SIZE) {
	try {
...
async function handleFilesUpload(files, onSend) {
	...
	try {
		...
		if (files.length === 1) {
			...
			// Generate preview for sender's own media files
			const mediaInfo = isMediaFile(file.name);
			if (mediaInfo.isImage || mediaInfo.isVideo) {
				const previewUrl = URL.createObjectURL(file);
				fileTransfer.previewUrl = previewUrl;
				fileTransfer.mediaType = mediaInfo.mediaType;
				fileTransfer.mimeType = getMimeType(file.name);
				console.log('[DEBUG] Generated sender preview:', previewUrl);
			}
...
async function sendVolumes(fileId, volumes, onSend, updateProgress, fileName) {
	...
	function sendNextBatch() {
		if (currentVolume >= volumes.length) {
			...
			// Inject preview for sender's own media files
			// 为发送方的媒体文件注入预览
			if (fileTransfer.previewUrl && fileTransfer.mediaType) {
				console.log('[DEBUG] Injecting sender preview after upload complete');
				const elements = document.querySelectorAll(`[data-file-id="${fileId}"]`);
				elements.forEach(element => {
					const fileMessage = element.closest('.file-message') || element;

					// Check if preview already exists
					if (fileMessage.querySelector('.file-media-preview')) {
						console.log('[DEBUG] Preview already exists, skipping');
						return;
					}

					// Create preview HTML
					let previewHtml = '';
					if (fileTransfer.mediaType === 'image') {
						previewHtml = `
							<div class="file-media-preview">
								<img src="${fileTransfer.previewUrl}" 
								     alt="${fileTransfer.fileName}" 
								     class="file-preview-image"
								     loading="lazy">
							</div>
						`;
					} else if (fileTransfer.mediaType === 'video') {
						previewHtml = `
							<div class="file-media-preview">
								<video controls class="file-preview-video" preload="metadata">
									<source src="${fileTransfer.previewUrl}" type="${fileTransfer.mimeType}">
					...
					// Insert preview at the beginning of file-message
					if (previewHtml) {
						console.log('[DEBUG] Inserting sender preview HTML');
						fileMessage.insertAdjacentHTML('afterbegin', previewHtml);
					}
				});
			}
...
function handleFileComplete(message) {
	...
	if (transfer.receivedVolumes.size === transfer.totalVolumes) {
		...
		console.log('[DEBUG] File transfer completed:', fileId, transfer.fileName);

		// Automatically generate media preview for images and videos
		// 自动为图片和视频生成媒体预览
		setTimeout(() => {
			console.log('[DEBUG] Calling generateMediaPreview for:', fileId);
			generateMediaPreview(fileId);
...
function isMediaFile(fileName) {
	...
	return {
		...
		mediaType: imageExts.includes(ext) ? 'image' : (videoExts.includes(ext) ? 'video' : null)
	};
}
...
// Decompress volumes to Blob without triggering download
// 将分卷解压为 Blob 而不触发下载
async function decompressVolumesToBlob(volumes) {
	try {
		// Combine all volumes using base64 decoding
		const combinedData = volumes.map(volume => {
			return base64ToArrayBuffer(volume);
		...
		console.error('Decompression to blob error:', error);
		throw error;
	}
}

// Generate media preview for completed file transfer
// 为已完成的文件传输生成媒体预览
async function generateMediaPreview(fileId) {
	console.log('[DEBUG] generateMediaPreview called for:', fileId);

	const transfer = window.fileTransfers.get(fileId);
	if (!transfer || transfer.status !== 'completed') {
		console.log('[DEBUG] Transfer not found or not completed:', transfer?.status);
		return;
	}
	...
	const mediaInfo = isMediaFile(transfer.fileName);
	console.log('[DEBUG] Media info for', transfer.fileName, ':', mediaInfo);

	if (!mediaInfo.isImage && !mediaInfo.isVideo) {
		console.log('[DEBUG] Not a media file, skipping preview');
		return;
	}

	try {
		console.log('[DEBUG] Starting decompression...');
		// Decompress file data to memory
		const decompressedData = await decompressVolumesToBlob(transfer.volumeData);
		console.log('[DEBUG] Decompression complete, size:', decompressedData.length);

		// Create Blob URL
		const mimeType = getMimeType(transfer.fileName);
		const blob = new Blob([decompressedData], { type: mimeType });
		const previewUrl = URL.createObjectURL(blob);
		console.log('[DEBUG] Created blob URL:', previewUrl);

		// Store preview information
		transfer.previewUrl = previewUrl;
		transfer.mediaType = mediaInfo.mediaType;
		transfer.mimeType = mimeType;

		// Update the file message UI to show the preview
		const elements = document.querySelectorAll(`[data-file-id="${fileId}"]`);
		console.log('[DEBUG] Found', elements.length, 'elements with file-id:', fileId);

		elements.forEach((element, index) => {
			console.log('[DEBUG] Processing element', index);

			// Find the file-message container
			const fileMessage = element.closest('.file-message') || element;
			console.log('[DEBUG] File message element:', fileMessage);

			// Check if preview already exists
			if (fileMessage.querySelector('.file-media-preview')) {
				console.log('[DEBUG] Preview already exists in DOM, skipping');
				return;
			}
			...
			if (transfer.mediaType === 'image') {
				...
				`;
			} else if (transfer.mediaType === 'video') {
				previewHtml = `
					<div class="file-media-preview">
						<video controls class="file-preview-video" preload="metadata">
			...
			// Insert preview at the beginning of file-message
			if (previewHtml) {
				console.log('[DEBUG] Inserting preview HTML');
				fileMessage.insertAdjacentHTML('afterbegin', previewHtml);
				console.log('[DEBUG] Preview HTML inserted successfully');
			}
		});
	} catch (error) {
		console.error('[ERROR] Failed to generate media preview:', error);
		// Silently fail - user can still download the file
	}
}
